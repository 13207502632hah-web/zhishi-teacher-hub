  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const [lessonsData, classesData] = await Promise.all([
        requestJson<{ lessons?: Lesson[] }>(`/api/lessons?${params}`, { signal }),
        requestJson<{ classes?: ClassRow[] }>("/api/classes?status=active", { signal }),
      ]);
      if (!lessonsData || !classesData) throw new HttpError(200, "课时或班级数据为空");
  ...
  return <AppShell title="课时记录" subtitle="把每一节课的准备、进展与课后记录放在同一条教学轨迹上" actions={<button className="primaryButton" disabled={submitting} onClick={() => edit()}>＋ 新建课时</button>}>
    ...
    <Panel className="lessonFilterPanel" eyebrow="课时检索" title="找到需要处理的课" description="输入关键词后再搜索，避免每次输入都重复请求。">
      <form className="lessonToolbar" onSubmit={(event) => { event.preventDefault(); setQuery(searchInput.trim()); }}>
        ...
        <label>班级<select aria-label="筛选班级" value={classFilter} onChange={(event) => setClassFilter(event.target.value)}><option value="">全部班级</option>{classes.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
    ...
    {view === "week" ? <Panel className="weekCalendarPanel" eyebrow="一周安排" title="按天查看课时"><div className="weekCalendar">{weekDays.map((date, index) => <article key={date}><header><b>周{["一","二","三","四","五","六","日"][index]}</b><span>{date.slice(5)}</span></header>{shown.filter((item) => item.date === date).length ? shown.filter((item) => item.date === date).map((item) => <Link href={`/lessons/${item.id}`} key={item.id}><time>{item.startTime || "待定"}</time><b>{item.topic || item.courseName}</b><StatusBadge tone={statusTone(item.status)}>{statuses[item.status] || item.status}</StatusBadge></Link>) : <p>无课时</p>}</article>)}</div></Panel> : view === "list" ? <Panel className="lessonListPanel" eyebrow="课程安排" title="课时列表" description="按日期查看状态、班级与授课方式。">{shown.length === 0 ? <EmptyState title="没有符合条件的课时" description="调整筛选条件，或创建一节真实课程。" action={<button className="secondaryButton" onClick={() => edit()}>新建第一节课</button>} /> : <div className="recordList">{shown.map((item) => <article key={item.id}><div className="dateBlock"><b>{item.date.slice(8)}</b><span>{item.date.slice(0, 7)}</span></div><div className="recordInfo"><StatusBadge tone={statusTone(item.status)}>{statuses[item.status] || item.status}</StatusBadge><h3><Link href={`/lessons/${item.id}`}>{item.courseName} · {String(item.topic || "未填写课题")}</Link></h3><p>{String(item.startTime || "待定")}–{String(item.endTime || "待定")}　{item.grade}　{item.className || "未关联班级"}　{item.mode === "online" ? "线上" : String(item.location || "线下")}</p></div><div className="rowActions"><Link href={`/lessons/${item.id}`}>详情</Link><button disabled={submitting} onClick={() => duplicate(item)}>复制</button><button disabled={submitting} onClick={() => edit(item)}>编辑</button><button disabled={submitting} onClick={() => remove(item.id)}>删除</button></div></article>)}</div>}</Panel> : <Panel className="lessonCalendarPanel" eyebrow="月度视图" title={month} description="点击课时进入详情；同一天的多节课会依次排列。"><div className="lessonCalendar"><div className="calendarWeek">{["日", "一", "二", "三", "四", "五", "六"].map((day) => <span key={day}>周{day}</span>)}</div><div className="calendarGrid">{days.map((day, index) => day == null ? <i key={`blank-${index}`}></i> : <article key={day}><b>{day}</b>{shown.filter((item) => Number(item.date.slice(8)) === day).map((item) => <Link href={`/lessons/${item.id}`} key={item.id}>{item.startTime || "待定"} {item.topic || item.courseName}</Link>)}</article>)}</div></div></Panel>}
