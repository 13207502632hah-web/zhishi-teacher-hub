  ...
  return <>
    ...
    <div className="v2-grid">
      <section className="v2-card"><header className="v2-card-head"><div><h2>现在值得处理</h2><p>按教学影响与紧迫程度排列</p></div><Link href="/v2/assistant">让 AI 重新分析</Link></header><div className="v2-action-list">
        ...
        <Link href="/v2/schedule-imports"><i>日</i><span><b>导入或核对新课表</b><small>自动识别日期、节次、学生、费用，并在写入前检查冲突</small></span><em>开始 →</em></Link>
        <Link href="/v2/questions"><i>题</i><span><b>用一句话找出合适的题</b><small>关键词结果先出现，语义重排随后渐进补齐</small></span><em>检索 →</em></Link>
      </div></section>
