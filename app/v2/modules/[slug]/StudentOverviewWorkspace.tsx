...
export function StudentOverviewWorkspace() {
  ...
  const loadStudents = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const endpoint = attentionMode
        ? "/api/v2/students/attention"
        : `/api/v2/students?${query.toString()}`;
  ...
  const save = async () => {
    ...
    setMessage("");
    try {
      const data = await requestJson<{ student?: Student }>("/api/v2/students", {
        method: "POST",
  ...
  return (
    <>
      <div className="studentOverviewPage">
        ...
        <nav className="studentGrowthRail" aria-label="学生成长视图">
          <button
          ...
          {canWrite && <Button onClick={openEditor}>录入学生</Button>}
        </nav>
              ...
              <div className={attentionMode ? "studentAttentionList" : "studentGrowthGrid"}>
                {rows.map((student) => attentionMode ? (
                  <article className="studentAttentionCard" key={student.id}>
                    ...
                    <Link className="zs-button zs-button--secondary" href={`/v2/detail/students/${student.id}`}>查看成长证据</Link>
                  </article>
                ) : (
                  <Link className="studentGrowthCard" href={`/v2/detail/students/${student.id}`} key={student.id}>
                    <header>
                      <span className="studentAvatar" aria-hidden="true">{personInitial(student.name)}</span>
                      <div>
            ...
            >
              <fieldset>
                ...
                <div className="studentFormGrid">
                  ...
                  <ClassPicker endpoint="/api/v2/classes/options" value={form.classId} onChange={(value) => setForm({ ...form, classId: value })} placeholder="暂不分班" />
