...
export function ClassOverviewWorkspace() {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const data = await requestJson<{
        ...
        `/api/v2/classes?${params.toString()}`,
        { signal },
      );
  ...
  const save = async () => {
    ...
    try {
      const data = await requestJson<{ class?: ClassRow }>(
        editing ? `/api/v2/classes/${editing}` : "/api/v2/classes",
        {
  ...
  const archive = async (row: ClassRow) => {
    ...
    setMessage("");
    try {
      const data = await requestJson<{ class?: ClassRow }>(`/api/v2/classes/${row.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      <div className="classOverviewPage">
        ...
        {canWrite && <div className="classOverviewHeaderActions"><Link className="zs-button zs-button--secondary" href="/v2/modules/students?new=1">录入学生</Link><Button onClick={() => openEditor()}>新建班级</Button></div>}
        ...
        <nav className="classRosterRail" aria-label="班级与学生视图">
          ...
          <Link href="/v2/modules/students"><span>个人成长</span><b>学生档案</b></Link>
        </nav>
                ...
                {rows.map((row) => {
                  ...
                  return (
                    <article className="classOverviewCard" key={row.id}>
                      ...
                      <footer>
                        <Link className="zs-button zs-button--primary" href={`/v2/detail/classes/${row.id}`}>查看班级</Link>
