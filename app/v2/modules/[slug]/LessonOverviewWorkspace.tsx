...
export function LessonOverviewWorkspace() {
  ...
  useEffect(() => {
    const params = new URLSearchParams(location.search), id = params.get("edit"), classId = params.get("classId") || params.get("class"), initialStatus = params.get("status"), from = params.get("from");
    if (initialStatus) setStatus(initialStatus);
    if (classId) setClassFilter(classId);
    if (from) setMonth(from.slice(0, 7));
    if (params.get("focus") === "post") setView("list");
    if (id) void requestJson<{ lesson?: Lesson }>(`/api/v2/lessons/${id}`).then((data) => data?.lesson && edit(data.lesson)).catch((reason) => setMessage(reason instanceof HttpError ? reason.message : "暂时无法读取课时详情"));
  ...
  const save = async (nextStatus?: string) => {
    ...
    try {
      ...
      const data = await requestJson<{ lesson?: Lesson }>(editing ? `/api/v2/lessons/${editing}` : "/api/v2/lessons", { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
  ...
  const remove = async (id: number) => {
    ...
    setMessage("");
    try {
      const data = await requestJson<{ ok?: boolean }>(`/api/v2/lessons/${id}`, { method: "DELETE" });
