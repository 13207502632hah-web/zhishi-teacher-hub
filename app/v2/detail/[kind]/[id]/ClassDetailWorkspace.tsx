...
export function ClassDetailWorkspace({ classId }: { classId: number }) {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    setDetailLoadError("");
    try {
      const result = await requestJson<ClassData>(`/api/v2/classes/${id}`, { signal });
  ...
  const loadCandidates = useCallback(async (signal?: AbortSignal) => {
    ...
    setCandidateLoadError("");
    try {
      const result = await requestJson<{ students?: Student[] }>("/api/v2/students?status=active", { signal });
  ...
  const add = async () => {
    ...
    setMessage("");
    try {
      const result = await requestJson<{ ok?: boolean }>(`/api/v2/classes/${id}`, {
        method: "POST",
  ...
  return (
    <>
      <section className="v2-detail-head"><div><Link href="/v2/modules/students?view=classes">← 返回班级册</Link><p className="v2-eyebrow">CLASS EVIDENCE PROFILE · #{id}</p><h2>{value.name}</h2><span>{value.stage} · {value.grade} · {value.courseType || "未设置课程类型"}</span></div>{canScheduleLesson && <Link className="v2-primary" href={`/v2/modules/students?view=lessons&new=1&classId=${id}`}>安排课时</Link>}</section>
      <div className="classDetailPage">
        ...
        <section className="classDetailIdentity" aria-label="班级档案摘要">
          ...
          <div className="classDetailIdentityActions">
            ...
            <Link href="/v2/modules/students?view=classes">返回班级册</Link>
          </div>
        </section>
        ...
        <div className="classDetailGrid">
          <Panel
            ...
            actions={canWrite ? (
              <Link className="classDetailTextLink" href="/v2/modules/students?new=1">录入新学生</Link>
            ) : undefined}
          >
            {!canWrite ? (
            ...
            {data.members.length === 0 ? (
              <EmptyState
                ...
                action={canWrite ? <Link href="/v2/modules/students?new=1" className="zs-button zs-button--secondary">录入学生</Link> : undefined}
              />
            ) : (
              <div className="classDetailMemberList">
                {data.members.map((student) => (
                  <article key={student.id}>
                    ...
                    <Link href={`/v2/detail/students/${student.id}`}>
          ...
          >
            {data.lessons.length === 0 ? (
              <EmptyState
                ...
                action={canScheduleLesson ? <Link className="zs-button zs-button--secondary" href={`/v2/modules/students?view=lessons&new=1&classId=${id}`}>安排课时</Link> : undefined}
              />
            ) : (
              <div className="classDetailTimeline">
                {data.lessons.slice(0, 6).map((lesson) => {
                  ...
                  return (
                    <Link href={`/v2/detail/lessons/${lesson.id}`} key={lesson.id}>
          ...
          <Panel
            ...
            actions={canWrite ? <Link className="classDetailTextLink" href={`/v2/operations?tab=assessments&classId=${id}`}>管理测验</Link> : undefined}
          >
            {data.assessments.length === 0 ? (
              <EmptyState
                title="还没有测验"
                description="新建测验并录入成绩后，这里会显示班级变化。"
                action={canWrite ? <Link className="zs-button zs-button--secondary" href={`/v2/operations?tab=assessments&classId=${id}`}>新建测验</Link> : undefined}
              />
            ) : (
              <div className="classDetailAssessmentList">
                {data.assessments.map((item) => (
                  <Link href={`/v2/operations/assessments/${item.id}`} key={item.id}>
