...
type Prep = { lesson?: Record<string, any>; previousLesson?: Record<string, any> | null; previousReflection?: Record<string, any> | null; pendingAssignments: Array<Record<string, any>>; attentionStudents: Array<Record<string, any>>; recommendedQuestions: Array<Record<string, any>> };
type AiLessonPrep = { teachingGoals: string; keyPoints: string; difficultPoints: string; materials: string; lessonFlow: string; questionUsePlan: string; evidenceSummary: string[]; uncertainty: string[] };
type AiReschedule = { summary: string; options: Array<{ candidateId: string; date: string; startTime: string; endTime: string; mode: string; location: string; priority: string; reason: string; tradeoffs: string[] }>; teacherChecks: string[]; uncertainty: string[] };
type WorkflowTemplate = { id: number; type: string; name: string; payload: Record<string, any> };
const empty: Activity = { members: [], assignments: [], feedback: [], reflections: [], questions: [], finance: null };
const emptyPrep: Prep = { pendingAssignments: [], attentionStudents: [], recommendedQuestions: [] };
const blankClosure = { actualContent: "", homework: "", nextPlan: "", participation: "", understanding: "", completion: "", discipline: "" };

export function LessonDetailWorkspace({ lessonId }: { lessonId: number }) {
  const id = String(lessonId);
  const [lesson, setLesson] = useState<Lesson | null>(null), [activity, setActivity] = useState<Activity>(empty), [prep, setPrep] = useState<Prep>(emptyPrep);
  const [prepForm, setPrepForm] = useState({ teachingGoals: "", keyPoints: "", difficultPoints: "", materials: "", knowledgePoints: "" }), [selectedQuestions, setSelectedQuestions] = useState<number[]>([]);
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const [lessonData, activityData, prepData, workflowData, templateData] = await Promise.all([
        requestJson<{ lesson?: Lesson }>(`/api/v2/lessons/${id}`, { signal }),
        requestJson<Activity>(`/api/v2/lessons/${id}/activity`, { signal }),
        requestJson<Prep>(`/api/v2/lessons/${id}/prep`, { signal }),
        requestJson<{ state?: { payload?: Record<string, any>; revision?: number } }>(`/api/v2/lessons/${id}/workflow-state`, { signal }),
        requestJson<{ templates?: WorkflowTemplate[] }>("/api/v2/workflow-templates", { signal }),
      ]);
  ...
  const persistDraft = useCallback(async (keepalive = false) => {
    ...
    try {
      const data = await requestJson<{ revision?: number }>(`/api/v2/lessons/${id}/workflow-state`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ revision: revisionRef.current, payload: payload() }), keepalive, timeoutMs: keepalive ? 0 : undefined });
  ...
  const setAssignmentField = (key: string, value: string) => { setAssignment((current) => ({ ...current, [key]: value })); setDirty(true); };
  const setFeedbackField = (key: string, value: string) => { setFeedback((current) => ({ ...current, [key]: value })); setDirty(true); };
  ...
  const saveWorkflow = async (action: "saveDraft" | "completeLesson") => {
    ...
    try {
      ...
      const data = await requestJson<Record<string, any>>(`/api/v2/lessons/${id}/activity`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  const savePrep = async () => {
    ...
    setBusy(true); setMessage("");
    try {
      const data = await requestJson<Record<string, any>>(`/api/v2/lessons/${id}/prep`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(prepForm) });
  ...
  const generateAiPrep = async () => {
    ...
    try {
      const data = await requestJson<Record<string, any>>("/api/v2/ai/lesson-prep", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lessonId: Number(id) }) });
  ...
  const generateAiReschedule = async () => {
    ...
    try {
      const data = await requestJson<Record<string, any>>("/api/v2/ai/schedule-reschedule", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lessonId: Number(id) }) });
  ...
  const addQuestions = async (target: "class" | "homework") => {
    ...
    setBusy(true); setMessage("");
    try {
      const url = target === "class" ? `/api/v2/lessons/${id}/questions/batch` : `/api/v2/lessons/${id}/homework-draft`;
  ...
  const saveTemplate = async (type: string, templatePayload: Record<string, unknown>) => {
    ...
    setBusy(true); setMessage("");
    try {
      const data = await requestJson<Record<string, any>>("/api/v2/workflow-templates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, name, payload: templatePayload }) });
  ...
  const undoCompletion = async () => {
    ...
    setBusy(true); setMessage("");
    try {
      const data = await requestJson<Record<string, any>>(`/api/v2/lessons/${id}/activity`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "undoLatestCompletion" }) });
  ...
  const printLesson = async () => {
    ...
    setBusy(true); setMessage("");
    try {
      await requestJson<Record<string, any>>("/api/v2/audit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "print", entityType: "lesson", entityId: id }) });
      window.print();
    } catch (reason) {
  ...
  if (!lesson && detailLoadError) return <><section className="v2-detail-head"><div><Link href="/v2/modules/students?view=lessons">← 返回课时列表</Link><p className="v2-eyebrow">V2 NATIVE LESSON DETAIL</p><h2>课时详情</h2></div></section><div className="lessonDetailLoadError" role="alert"><EmptyState title="无法打开课时" description={detailLoadError} action={<button className="secondaryButton" onClick={() => setReloadKey((value) => value + 1)}>重新读取课时详情</button>} /></div></>;
  if (!lesson) return <><section className="v2-detail-head"><div><Link href="/v2/modules/students?view=lessons">← 返回课时列表</Link><p className="v2-eyebrow">V2 NATIVE LESSON DETAIL</p><h2>课时详情</h2></div></section><EmptyState title="正在读取课时" description="请稍候…" /></>;
  ...
  return <><section className="v2-detail-head"><div><Link href="/v2/modules/students?view=lessons">← 返回课时列表</Link><p className="v2-eyebrow">V2 NATIVE LESSON DETAIL · #{id}</p><h2>{String(lesson.displayTitle || lesson.topic || lesson.courseName)}</h2><span>{lesson.date} · {lesson.grade} · {lesson.status === "completed" ? "已完成" : "待记录"}</span></div><div className="v2-hero-actions"><Link href={`/v2/modules/students?view=lessons&edit=${id}`} className="v2-secondary-on-light">编辑课时</Link>{lesson.status === "completed" && <button className="v2-secondary-on-light" disabled={busy} onClick={undoCompletion}>撤销本次完成</button>}<button className="v2-primary" disabled={busy} onClick={printLesson}>打印 / 导出 PDF</button></div></section>
    ...
    {completionResult && <section className="panel completionReceipt"><div className="panelTitle"><div><p>本次完成结果</p><h2>已经生成和仍需处理的内容</h2></div></div><div className="receiptGrid"><Link href={`/v2/modules/assignments?lessonId=${id}`}>作业：{completionResult.artifacts?.assignmentId ? "已生成草稿" : "未生成"}</Link><Link href={`/v2/modules/learning?view=feedback&lessonId=${id}`}>反馈：{completionResult.artifacts?.feedbackId ? "已生成草稿" : "未生成"}</Link><Link href="/v2/modules/finance">结算：{completionResult.artifacts?.financeId ? completionResult.artifacts.financeStatus : "未生成"}</Link><span>剩余待办：{completionResult.todos?.length ? completionResult.todos.join("、") : "无"}</span></div></section>}

    {!['completed', 'cancelled'].includes(String(lesson.status)) && <section className="panel aiReviewPanel"><div className="panelTitle"><div><p>DeepSeek · 课表辅助</p><h2>从真实空档中推荐调课方案</h2></div><div className="cardActions"><button className="aiButton" disabled={aiRescheduleBusy || !lesson.startTime || !lesson.endTime} onClick={generateAiReschedule}>{aiRescheduleBusy ? "正在核对课表…" : aiReschedule ? "重新生成" : "生成 AI 调课建议"}</button><Link className="secondaryButton" href={`/v2/modules/students?view=lessons&edit=${id}`}>进入课时编辑</Link></div></div><p className="privacyNote">系统先按现有课表排除时间冲突，再让 AI 对真实候选空档排序；不会发送学生姓名、自动改课或替教师确认学生与场地。</p>{!lesson.startTime || !lesson.endTime ? <p className="emptyInline">请先在课时编辑中补齐开始和结束时间。</p> : aiReschedule && <><h3>{aiReschedule.summary}</h3><div className="prepContextGrid">{aiReschedule.options.map((option) => <article key={option.candidateId}><b>{option.priority} · {option.date}</b><span>{option.startTime}–{option.endTime} · {option.mode === "online" ? "线上" : option.location || "地点待确认"}</span><p>{option.reason}</p>{option.tradeoffs.length > 0 && <small>取舍：{option.tradeoffs.join("；")}</small>}</article>)}</div>{[...aiReschedule.teacherChecks, ...aiReschedule.uncertainty].length > 0 && <div className="aiUncertainty"><b>调课前必须人工确认</b>{[...aiReschedule.teacherChecks, ...aiReschedule.uncertainty].map((item) => <span key={item}>{item}</span>)}</div>}<button className="secondaryButton" onClick={() => navigator.clipboard.writeText([aiReschedule.summary, ...aiReschedule.options.map((option) => `${option.priority}｜${option.date} ${option.startTime}–${option.endTime}｜${option.location || "地点待确认"}\n${option.reason}`), `需确认：${aiReschedule.teacherChecks.join("；")}`].join("\n\n"))}>复制调课建议</button>{aiRescheduleMeta && <details><summary>查看本次发送与排除字段</summary><p>发送：{aiRescheduleMeta.sentFields.join("、")}</p><p>排除：{aiRescheduleMeta.excludedFields.join("、")}</p></details>}</>}</section>}
