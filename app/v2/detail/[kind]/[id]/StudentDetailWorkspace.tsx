...
export function StudentDetailWorkspace({ studentId }: { studentId: number }) {
  ...
  const loadArchive = useCallback(
    async (signal: AbortSignal) => {
      ...
      try {
        const payload = await requestJson<StudentData>(`/api/v2/students/${encodedId}`, { signal });
  ...
  const loadMastery = useCallback(
    async (signal: AbortSignal) => {
      ...
      try {
        const payload = await requestJson<{ mastery?: Mastery | null }>(`/api/v2/students/${encodedId}/mastery`, { signal });
  ...
  const loadInsights = useCallback(
    async (signal: AbortSignal) => {
      ...
      try {
        const payload = await requestJson<Insights>(`/api/v2/students/${encodedId}/insights?weeks=4`, { signal });
  ...
  const loadTrends = useCallback(
    async (signal: AbortSignal) => {
      ...
      try {
        const payload = await requestJson<ScoreTrends>(`/api/v2/students/${encodedId}/score-trends`, { signal });
  ...
  const openWrongQuestion = useCallback(async () => {
    ...
    try {
      const payload = await requestJson<{ results?: Row[] }>("/api/v2/questions/search", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query: "", mode: "keyword", phase: "lexical", pageSize: 50, useCase: "remediation", filters: { status: "active" } }) });
      if (!payload) throw new Error("服务器返回空响应，无法读取正式题库");
      const next = { questionId: "", incorrectAnswer: "", reason: "" };
      setQuestionChoices(rows(payload.results));
      setWrongForm(next);
  ...
  const save = useCallback(async () => {
    ...
    setSaveBusy(true);
    try {
      const payload = await requestJson<{ student?: Student }>(`/api/v2/students/${encodedId}`, {
        method: "PUT",
  ...
  const archive = useCallback(async () => {
    ...
    setArchiveBusy(true);
    try {
      const payload = await requestJson<{ ok?: boolean }>(`/api/v2/students/${encodedId}`, { method: "DELETE" });
  ...
  const saveWrongQuestion = useCallback(async () => {
    ...
    try {
      const payload = await requestJson<{ updated?: boolean }>(`/api/v2/students/${encodedId}/wrong-questions`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const saveMastery = useCallback(async () => {
    ...
    try {
      const payload = await requestJson<{ mastery?: Mastery }>(`/api/v2/students/${encodedId}/mastery`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const setWrongStatus = useCallback(
    async (wrongQuestionId: number, status: "active" | "mastered") => {
      ...
      try {
        const payload = await requestJson<{ ok?: boolean }>(`/api/v2/students/${encodedId}/wrong-questions`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const reveal = useCallback(async () => {
    ...
    setPrivateBusy(true);
    try {
      const payload = await requestJson<{ guardianContact?: string }>(`/api/v2/students/${encodedId}/private`, { timeoutMs: 10_000 });
  ...
  const createReport = useCallback(async () => {
    ...
    setReportError(null);
    try {
      const payload = await requestJson<Report>(`/api/v2/students/${encodedId}/monthly-report`);
  ...
  const loadRecommendations = useCallback(async () => {
    ...
    try {
      const payload = await requestJson<Recommendations>(`/api/v2/students/${encodedId}/recommendations`);
  ...
  const generateRemediation = useCallback(async () => {
    ...
    try {
      ...
      }>("/api/v2/ai/wrong-question-remediation", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const timelineItems = useMemo<TimelineItem[]>(() => {
    ...
    const fallbackItems: TimelineItem[] = [
      ...data.lessonRecords.map((item) => ({
        ...
        href: `/v2/detail/lessons/${text(item.lesson_id)}`,
      })),
      ...data.submissions.map((item) => ({
        ...
        href: `/v2/modules/assignments?q=${encodeURIComponent(text(item.title))}`,
      })),
      ...data.feedback.map((item) => ({
        ...
        href: `/v2/modules/learning?view=feedback&studentId=${encodedId}&status=${encodeURIComponent(text(item.status))}`,
      })),
      ...data.results.map((item) => ({
        ...
        href: "/v2/operations?tab=assessments",
      })),
  ...
  const headerActions = (
    <>
      ...
      <a className={styles.headerButton} href={`/v2/modules/learning?view=feedback&new=1&type=stage&studentId=${encodedId}`}>
        生成阶段总结
      </a>
  ...
  if (!data && sectionStates.archive.loading) {
    return <div className={styles.page}><LoadingBlock label="正在读取学生完整成长档案" /></div>;
  }

  if (!data && sectionStates.archive.error) {
    return <div className={styles.page}><SectionError message={sectionStates.archive.error} onRetry={() => retrySection("archive")} label="重新读取档案" /></div>;
  }
  ...
  return (
    <>
      <section className="v2-detail-head"><div><Link href="/v2/modules/students">← 返回学生工作台</Link><p className="v2-eyebrow">STUDENT EVIDENCE PROFILE · #{encodedId}</p><h2>{studentName}</h2><span>{text(student.grade, "年级待补")} · 学习成长档案</span></div><div className={styles.actionRow}>{headerActions}</div></section>
      <div className={styles.page}>
        ...
        <div className={styles.twoColumn}>
          ...
          <section className={styles.section} aria-labelledby="recommendation-title">
            ...
            {recommendationsError ? <SectionError message={recommendationsError} onRetry={() => void loadRecommendations()} label="重新生成推荐" /> : recommendations?.questions?.length ? <div className={styles.recommendationList}>{recommendations.questions.map((question, index) => <article key={`${text(question.id)}-${index}`}><div><span className={styles.statusBadge}>{text(question.difficulty, "题目")} · {text(question.questionType, "正式题")}</span><h3>{text(question.stem, "题干待补")}</h3><p>{text(question.knowledgePoints, "知识点待补")}</p></div><Link className={styles.buttonQuiet} href={`/v2/questions?q=${encodeURIComponent(text(question.stem).slice(0, 20))}`}>教师确认</Link></article>)}</div> : recommendations ? <DataNote title="暂无匹配题目" description={text(recommendations.message, "正式题库中暂时没有匹配题目，请先补充知识点题目。")}/> : <DataNote title="尚未生成推荐" description="系统只从正式题库选择基础、提高和综合题；题量不足时会明确提示。" />}
            <div className={styles.stageFeedback}><div><strong>阶段反馈</strong><p>基于本页事实记录生成教师阶段总结草稿。</p></div><a className={styles.buttonSecondary} href={`/v2/modules/learning?view=feedback&new=1&type=stage&studentId=${encodedId}`}>生成阶段总结</a></div>
          </section>
        </div>
      </div>
