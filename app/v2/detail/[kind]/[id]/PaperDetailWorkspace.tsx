...
export function PaperDetailWorkspace({ paperId }: { paperId: number }) {
  ...
  const assignmentOperationIdRef = useRef("");
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    const requests = await Promise.allSettled([
      requestJson<PaperData>(`/api/v2/papers/${id}`, { signal }),
      requestJson<{ files?: Row[] }>(`/api/v2/papers/${id}/files`, { signal }),
    ]);
  ...
  const dismissAssignment = useCallback(() => {
    ...
    assignmentOperationIdRef.current = "";
  }, [actionBusy, assignmentDirty]);

  const openAssignment = () => {
    assignmentOperationIdRef.current = crypto.randomUUID();
    setAssignOpen(true);
  };
  ...
  const print = async () => {
    ...
    setMessage("");
    try {
      await requestJson("/api/v2/audit", {
        method: "POST",
  ...
  const downloadDocx = async () => {
    ...
    setMessage("");
    try {
      const { blob, response } = await requestBlob(`/api/v2/papers/${id}/export?mode=${mode}`);
      const encodedName =
    ...
    try {
      ...
      if (job.status === "completed") {
        const cached = await requestBlob(
          `/api/v2/papers/${id}/export-job?jobId=${encodeURIComponent(job.jobId)}`,
        );
        blob = cached.blob;
      } else {
        ...
        await requestJson(`/api/v2/papers/${id}/export-job`, {
          method: "PUT",
          body: formData,
          timeoutMs: 45_000,
        });
      ...
      if (jobId) {
        void requestJson(`/api/v2/papers/${id}/export-job`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const copy = async () => {
    ...
    setMessage("");
    try {
      const payload = await requestJson<{ paper?: Row }>(`/api/v2/papers/${id}`, {
        method: "POST",
      });
  ...
  const updateStatus = async (status: string) => {
    ...
    setMessage("");
    try {
      await requestJson(`/api/v2/papers/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const assignPaper = async () => {
    ...
    setMessage("");
    try {
      const operationId = assignmentOperationIdRef.current || crypto.randomUUID();
      assignmentOperationIdRef.current = operationId;
      const payload = await requestJson<{ id?: number }>("/api/v2/assignments", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...assignment,
      ...
      if (!payload?.id) throw new HttpError(200, "作业草稿响应为空，请重试");
      await requestJson("/api/v2/approvals", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          actionType: "assignment.publish",
          entityType: "assignment",
          entityId: String(payload.id),
          title: `发布试卷作业：${data?.paper.title || "完整试卷"}`,
          summary: `面向所选班级发布整张试卷${assignment.dueAt ? `，截止 ${assignment.dueAt}` : ""}。批准后学生端立即可见。`,
          payload: { id: payload.id },
          evidence: [{ type: "paper", id: Number(id), title: data?.paper.title, classId: Number(assignment.classId), dueAt: assignment.dueAt }],
        }),
      });
      setAssignment(defaultAssignment());
      assignmentOperationIdRef.current = "";
      setAssignOpen(false);
      setMessage("作业草稿已建立并进入待确认中心；批准后才会同步给学生。");
      await load();
    } catch (reason) {
  ...
  return (
    <>
      <section className="v2-detail-head">
        <div>
          <Link href="/v2/modules/papers">← 返回组卷工作台</Link>
          <p className="v2-eyebrow">V2 NATIVE PAPER · #{id}</p>
          <h2>{String(paper.title)}</h2>
          <span>{paper.type} · {data.questions.length}题 · {totalScore}分</span>
        </div>
      ...
      <div className="paperDetailPage">
        ...
        {files.length > 0 && (
          <Panel
            ...
            eyebrow="原始文件"
            title="整张试卷版本"
            actions={<Button onClick={openAssignment}>布置为作业</Button>}
          >
            <div className="paperDetailFileList">
              {files.map((file) => (
                <article key={file.id}>
                  ...
                  <a className="zs-button zs-button--secondary" href={`/api/v2/papers/${id}/files/${file.id}?inline=1`} target="_blank" rel="noreferrer">打开并打印原卷</a>
                  <a className="zs-button zs-button--quiet" href={`/api/v2/papers/${id}/files/${file.id}`}>下载原文件</a>
                </article>
              ))}
            </div>
          </Panel>
