...
export function QuestionSearch() {
  ...
  const importJobId = importJob?.id, importJobState = importJob?.state;
  useEffect(() => { if (!importJobId || !["queued", "running"].includes(importJobState || "")) return; let cancelled = false, timer: number | undefined; const poll = async () => { try { await loadImport(importJobId); } catch (reason) { setError(reason instanceof Error ? reason.message : "任务刷新失败"); } finally { if (!cancelled) timer = window.setTimeout(poll, 2500); } }; timer = window.setTimeout(poll, 2500); return () => { cancelled = true; if (timer) window.clearTimeout(timer); }; }, [importJobId, importJobState, loadImport]);
