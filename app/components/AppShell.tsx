...
export function AppShell({ title, subtitle, actions, children, publicLanding = false }: { title: string; subtitle?: string; actions?: ReactNode; children: ReactNode; publicLanding?: boolean }) {
  ...
  if (publicPage) return <><a className="skipLink" href="#main-content">跳到主要内容</a><div className={`publicShell${publicLanding ? " publicShell--landing" : ""}`}><header className="publicHeader"><Link href="/" className="publicBrand"><span>{BRAND_MARK}</span><div><b>{BRAND_NAME}</b><small>{PUBLIC_TEACHER_SPACE}</small></div></Link><nav aria-label="公开导航"><Link href="/">首页</Link><Link href="/resources">公开资源</Link><Link href="/resources#teaching-method">教学理念</Link><Link className="workspaceEntry" href="/workspace">{session?.authenticated ? "进入工作台" : "教师登录"}</Link></nav></header>{!publicLanding && <div className="publicPageHead"><div><p>{BRAND_NAME} / {title}</p><h1>{title}</h1>{subtitle && <span>{subtitle}</span>}</div>{session?.authenticated && actions && <div className="headerActions">{actions}</div>}</div>}<main className={`publicContent${publicLanding ? " publicContent--landing" : ""}`} id="main-content">{children}</main><footer className="publicFooter"><b>{BRAND_NAME}</b><span>{BRAND_PUBLIC_SPACE}</span><Link href="/workspace">教师工作台</Link></footer></div></>;
