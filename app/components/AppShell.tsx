...
export function AppShell({ title, subtitle, actions, children, publicLanding = false }: { title: string; subtitle?: string; actions?: ReactNode; children: ReactNode; publicLanding?: boolean }) {
  ...
  const learnerPortal = pathname === "/portal";
  const learnerSession = session?.role === "student" || session?.role === "parent";
  ...
  useEffect(() => {
    const handleShortcut = (event: KeyboardEvent) => {
      if (!publicPage && !learnerSession && (event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") {
        event.preventDefault();
      ...
      if (commandOpen && event.key === "Tab") {
        const focusable = Array.from(commandDialogRef.current?.querySelectorAll<HTMLElement>("button:not([disabled]), input:not([disabled]), a[href]") || []);
      ...
      const button = document.querySelector<HTMLButtonElement>(".modalBackdrop .modalTitle button");
      if (button) {
        event.preventDefault();
        button.click();
      }
  ...
  }, [closeCommand, commandOpen, learnerSession, publicPage]);
  if (learnerPortal && !session?.authenticated) return <><a className="skipLink" href="#main-content">跳到主要内容</a><div className="publicShell"><header className="publicHeader"><Link href="/" className="publicBrand"><span>知</span><div><b>知师研室</b><small>学生与家长安全入口</small></div></Link><nav aria-label="门户导航"><Link href="/resources">公开资源</Link><span>请从微信小程序进入</span></nav></header><div className="publicPageHead"><div><p>知师研室 / 我的学习</p><h1>{title}</h1><span>{subtitle || "学生与家长移动门户"}</span></div></div><main className="publicContent" id="main-content">{children}</main><footer className="publicFooter"><b>知师研室</b><span>绑定与会话失效后立即停止展示学生资料</span><Link href="/resources">公开资源</Link></footer></div></>;
  ...
  const commandNavigation = [...visibleItems, ...visibleUtilities];
  const commandItems = commandNavigation.filter((item) => `${item.label} ${item.group} ${item.href}`.toLowerCase().includes(commandQuery.trim().toLowerCase()));
  return <><a className="skipLink" href="#main-content">跳到主要内容</a><div className="appShell">
    ...
    <div className="appMain">
      <header className="appHeader"><div><p>知师研室 / {title}</p><h1>{title}</h1>{subtitle && <span>{subtitle}</span>}</div><div className="headerActions">{(!publicPage || session?.authenticated) && actions}{!learnerSession && <><button className="commandTrigger" aria-label="快速跳转，快捷键 Command 或 Control 加 K" onClick={() => { setTodoOpen(false); setCommandOpen(true); }} ref={commandTriggerRef}><span>快速跳转</span><kbd>⌘K</kbd></button>{session?.authenticated && <button aria-controls="teaching-todo-popover" className="iconButton" aria-label="教学待办" aria-expanded={todoOpen} onClick={toggleTodos} ref={todoTriggerRef}>◌</button>}{todoOpen && <section aria-label="教学待办列表" className="todoPopover" id="teaching-todo-popover" ref={todoPopoverRef} role="dialog"><div><b>教学待办</b><button aria-label="关闭待办" onClick={closeTodos} ref={todoCloseRef}>×</button></div>{todoLoading && <p aria-live="polite" role="status">正在读取…</p>}{todoError && !todoLoading && <div className="todoPopover__error" role="alert"><p><b>待办读取失败</b><span>{todoError}</span></p><button disabled={todoLoading} onClick={() => void loadTodos()} type="button">重新读取</button></div>}{todos && !todoLoading && !todoError && <ul><li><Link href="/lessons" onClick={() => setTodoOpen(false)}>待处理课时</Link><b>{todos.draftLessons || 0}</b></li><li><Link href="/feedback" onClick={() => setTodoOpen(false)}>待确认反馈</Link><b>{todos.pendingFeedback || 0}</b></li><li><Link href="/questions?status=review" onClick={() => setTodoOpen(false)}>待校对题目</Link><b>{todos.pendingReview || 0}</b></li><li><Link href="/assignments" onClick={() => setTodoOpen(false)}>作业待批改</Link><b>{todos.pendingHomework || 0}</b></li></ul>}</section>}</>}</div></header>
      <main className="appContent" id="main-content">{children}</main>
    </div>
  </div>{commandOpen && !learnerSession && <div className="modalBackdrop quickSwitcherBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeCommand(); }}><section className="quickSwitcher" role="dialog" aria-modal="true" aria-label="快速跳转" ref={commandDialogRef}><div className="quickSwitcherSearch"><span aria-hidden="true">⌕</span><input autoFocus value={commandQuery} onChange={(event) => setCommandQuery(event.target.value)} placeholder="搜索题库、课时、学生、反馈或设置" aria-label="搜索工作台入口" /><button aria-label="关闭快速跳转" onClick={() => closeCommand()}>ESC</button></div><div className="quickSwitcherResults">{commandItems.length ? commandItems.map((item) => <Link key={item.href} href={item.href} onClick={() => closeCommand(false)}><NavigationIcon value={item.icon} /><span><b>{item.label}</b><small>{item.group}</small></span><em>打开</em></Link>) : <p>没有匹配入口，请尝试“题库”“课时”“学生”或“设置”。</p>}</div><footer><span>按 ⌘K / Ctrl+K 随时打开</span><span>ESC 关闭</span></footer></section></div>}</>;
}
