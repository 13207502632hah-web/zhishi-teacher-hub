...
import { BRAND_MARK, BRAND_NAME, BRAND_PUBLIC_SPACE, PUBLIC_TEACHER_SPACE } from "@/app/lib/brand";
...
export function AppShell({ title, subtitle, actions, children, publicLanding = false }: { title: string; subtitle?: string; actions?: ReactNode; children: ReactNode; publicLanding?: boolean }) {
  ...
  const publicPage = ["/", "/resources", "/privacy", "/terms", "/account-deletion", "/support"].includes(pathname) || /^\/resources\/\d+$/.test(pathname);
  const toggleTodos = async () => { const next = !todoOpen; setTodoOpen(next); if (next && !todos) { const response = await fetch("/api/v2/dashboard"); if (response.ok) setTodos(await response.json()); } };
  ...
  if (publicPage) return <><a className="skipLink" href="#main-content">跳到主要内容</a><div className={`publicShell${publicLanding ? " publicShell--landing" : ""}`}><header className="publicHeader"><Link href="/" className="publicBrand"><span>{BRAND_MARK}</span><div><b>{BRAND_NAME}</b><small>{PUBLIC_TEACHER_SPACE}</small></div></Link><nav aria-label="公开导航"><Link href="/">首页</Link><Link href="/resources">公开资源</Link><Link href="/resources#teaching-method">教学理念</Link><Link href="/support">帮助支持</Link><Link className="workspaceEntry" href="/v2">{session?.authenticated ? "进入工作台" : "教师登录"}</Link></nav></header>{!publicLanding && <div className="publicPageHead"><div><p>{BRAND_NAME} / {title}</p><h1>{title}</h1>{subtitle && <span>{subtitle}</span>}</div>{session?.authenticated && actions && <div className="headerActions">{actions}</div>}</div>}<main className={`publicContent${publicLanding ? " publicContent--landing" : ""}`} id="main-content">{children}</main><footer className="publicFooter"><div><b>{BRAND_NAME}</b><span>{BRAND_PUBLIC_SPACE}</span></div><nav aria-label="合规与帮助"><Link href="/privacy">隐私政策</Link><Link href="/terms">用户协议</Link><Link href="/account-deletion">账号删除</Link><Link href="/support">帮助支持</Link></nav><Link href="/v2">教师工作台</Link></footer></div></>;
  ...
  return <><a className="skipLink" href="#main-content">跳到主要内容</a><div className="appShell">
    ...
    <div className="appMain">
      <header className="appHeader"><div><p>{BRAND_NAME} / {title}</p><h1>{title}</h1>{subtitle && <span>{subtitle}</span>}</div><div className="headerActions">{(!publicPage || session?.authenticated) && actions}<button className="commandTrigger" aria-label="快速跳转，快捷键 Command 或 Control 加 K" onClick={() => setCommandOpen(true)}><span>快速跳转</span><kbd>⌘K</kbd></button>{session?.authenticated && <button className="iconButton" aria-label="教学待办" aria-expanded={todoOpen} onClick={toggleTodos}>◌</button>}{todoOpen && <section className="todoPopover" aria-label="教学待办列表"><div><b>教学待办</b><button aria-label="关闭待办" onClick={() => setTodoOpen(false)}>×</button></div>{!todos ? <p>正在读取…</p> : <ul><li><Link href="/v2/modules/students?view=lessons">待处理课时</Link><b>{todos.draftLessons || 0}</b></li><li><Link href="/v2/modules/learning?view=feedback">待确认反馈</Link><b>{todos.pendingFeedback || 0}</b></li><li><Link href="/v2/questions?view=library&status=review">待校对题目</Link><b>{todos.pendingReview || 0}</b></li><li><Link href="/v2/modules/assignments?submissionStatus=pending">作业待批改</Link><b>{todos.pendingHomework || 0}</b></li></ul>}</section>}</div></header>
