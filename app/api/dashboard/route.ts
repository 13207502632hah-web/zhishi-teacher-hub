...
export async function GET(request: Request) {
  ...
  for (const lesson of overdueLessons) actions.push({ key: `overdue-${lesson.id}`, type: "lesson-closeout", title: `补记：${lesson.displaySubject || lesson.topic || lesson.courseName}`, reason: "课时日期已过，课堂记录尚未完成", dueAt: lesson.date, href: `/lessons/${lesson.id}`, updatedAt: lesson.updatedAt });
  for (const lesson of todayLessons) if (lesson.status !== "completed") actions.push({ key: `today-${lesson.id}`, type: "today-lesson", title: `${lesson.prepReady ? "完成" : "备课"}：${lesson.displaySubject || lesson.topic || lesson.courseName}`, reason: lesson.prepReady ? "今天的课尚未完成课后记录" : "教学目标、重点或资料清单尚未补齐", dueAt: today, href: `/lessons/${lesson.id}` });
  if (nextLesson && !Number(nextLesson.prepReady) && nextLesson.date !== today) actions.push({ key: `next-${nextLesson.id}`, type: "lesson-prep", title: `准备下一节：${nextLesson.displaySubject || nextLesson.topic || nextLesson.courseName}`, reason: "下一节课的教学目标、重点或资料清单尚未补齐", dueAt: nextLesson.date, href: `/lessons/${nextLesson.id}` });
