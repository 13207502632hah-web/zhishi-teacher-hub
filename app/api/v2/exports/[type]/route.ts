...
export async function GET(request: Request, context: { params: Promise<{ type: string }> }) {
  ...
  if (type === "lessons") {
    ...
    headers = ["日期","开始","结束","课程","班级","课题","状态","实际内容","作业","下节计划"]; rows = (result.results as Record<string, unknown>[]).map((item) => [item.date,item.start_time,item.end_time,item.course_name,item.className,item.topic,item.status,item.actual_content,item.homework,item.next_plan]); label = "课时记录";
    ...
    headers = ["姓名","年级","学校","教材","考试目标","基础水平","优势","薄弱知识点","阶段目标","状态","班级"]; rows = (result.results as Record<string, unknown>[]).map((item) => [item.name,item.grade,item.school,item.textbook_version,item.exam_goal,item.foundation_level,item.strengths,item.weak_knowledge,item.stage_goal,item.status,item.classes]); label = "学生名单";
    ...
    headers = ["日期","测验","班级","类型","总分","学生","年级","得分","客观题","主观题","薄弱知识点","教师备注"]; rows = (result.results as Record<string, unknown>[]).map((item) => [item.date,item.title,item.className,item.type,item.total_score,item.name,item.grade,item.score,item.objective_score,item.subjective_score,item.weak_knowledge,item.teacher_note]); label = assessmentId ? "单次测验成绩" : "测验成绩";
  } else if (type === "assignments") {
    const result = await env.DB.prepare("SELECT l.date,a.title,c.name AS className,s.name,sub.status,sub.score,sub.teacher_note,sub.submitted_at FROM assignment_submissions sub JOIN assignments a ON a.id=sub.assignment_id LEFT JOIN lessons l ON l.id=a.lesson_id LEFT JOIN classes c ON c.id=COALESCE(a.class_id,l.class_id) JOIN students s ON s.id=sub.student_id ORDER BY l.date DESC,a.id,s.name").all();
    headers = ["课时日期","作业","班级","学生","状态","分数","教师备注","提交时间"]; rows = (result.results as Record<string, unknown>[]).map((item) => [item.date,item.title,item.className,item.name,item.status,item.score,item.teacher_note,item.submitted_at]); label = "作业完成情况";
