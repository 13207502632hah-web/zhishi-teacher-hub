...
const forIdChunks = <T>(values: number[], build: (placeholders: string, chunk: number[]) => T): T[] => {
  const unique = [...new Set(values.map(Number).filter(Number.isFinite))];
  const statements: T[] = [];
  for (let index = 0; index < unique.length; index += 80) {
    const chunk = unique.slice(index, index + 80);
    statements.push(build(marks(chunk), chunk));
  }
  return statements;
};
...
export async function DELETE(request: Request) {
  ...
  const statements = [
    ...
    ...forIdChunks(demoFeedbackIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM ai_feedback_learning_events WHERE feedback_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(demoFeedbackIds, (placeholders, chunk) => env.DB.prepare(`UPDATE feedback SET ai_draft_id=NULL WHERE id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(demoFeedbackIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM feedback_evidence WHERE feedback_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(lessonIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM ai_feedback_drafts WHERE lesson_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(studentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM ai_feedback_drafts WHERE student_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(demoFeedbackIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM ai_feedback_drafts WHERE feedback_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(demoFeedbackIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM feedback WHERE id IN (${placeholders})`).bind(...chunk)),
    ...
    // 历史演示课时可能已不在 demo_records 中，但仍引用本轮演示计价规则。
    // 保留这些课时账目并解除规则引用，避免清理计价规则时触发外键失败。
    env.DB.prepare(`UPDATE lesson_finance SET pricing_rule_id=NULL WHERE pricing_rule_id IN (${marks(pricingRuleIds)})`).bind(...pricingRuleIds),
    ...
    ...forIdChunks(demoProjectIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM exam_project_students WHERE project_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(studentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM exam_project_students WHERE student_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(demoResultIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM exam_project_students WHERE assessment_result_id IN (${placeholders})`).bind(...chunk)),
    ...
    ...forIdChunks(demoResultIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assessment_question_results WHERE assessment_result_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(questionIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assessment_question_results WHERE question_id IN (${placeholders})`).bind(...chunk)),
    env.DB.prepare(`DELETE FROM ai_question_reviews WHERE question_id IN (${marks(questionIds)})`).bind(...questionIds),

    // 作业与提交。
    ...forIdChunks(submissionReviewIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM review_assets WHERE review_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(submissionReviewIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM submission_reviews WHERE id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(submissionVersionIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM review_annotations WHERE submission_version_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(submissionVersionIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM excellent_submissions WHERE submission_version_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(submissionVersionIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM submission_assets WHERE submission_version_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(submissionVersionIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM submission_versions WHERE id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(assignmentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assignment_assets WHERE assignment_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(assignmentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assignment_targets WHERE assignment_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(assignmentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assignment_settings WHERE assignment_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(assignmentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assignment_submissions WHERE assignment_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(studentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assignment_submissions WHERE student_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(assignmentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assignments WHERE id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(lessonIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assignments WHERE lesson_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(classIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assignments WHERE class_id IN (${placeholders})`).bind(...chunk)),
    ...
    ...forIdChunks(demoResultIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assessment_results WHERE id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(demoAssessmentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assessment_results WHERE assessment_id IN (${placeholders})`).bind(...chunk)),
    ...forIdChunks(studentIds, (placeholders, chunk) => env.DB.prepare(`DELETE FROM assessment_results WHERE student_id IN (${placeholders})`).bind(...chunk)),
