...
import { revokeStaffSessions, setStaffPassword } from "../../../lib/staff-auth";
import { passwordStrengthError } from "../../../lib/teacher-auth-policy";

const allowedRoles: RoleCode[] = ["assistant", "student", "parent"];

export async function GET() {
  const access = await requirePermission("settings:read"); if (isDenied(access)) return access;
  if (access.authType !== "teacher_admin") return Response.json({ error: "只有主教师可以管理工作室成员" }, { status: 403 });
  const [users, students, classes, staffClassAccess, logs] = await Promise.all([
    env.DB.prepare("SELECT u.id,u.name,u.email,u.status,GROUP_CONCAT(r.code) AS roles,GROUP_CONCAT(r.name) AS roleNames,EXISTS(SELECT 1 FROM staff_credentials sc WHERE sc.user_id=u.id) AS hasCredential FROM users u LEFT JOIN user_roles ur ON ur.user_id=u.id LEFT JOIN roles r ON r.id=ur.role_id WHERE COALESCE(u.email,'') NOT LIKE '%@chatgpt.com' AND COALESCE(u.email,'') NOT LIKE '%@local.invalid' GROUP BY u.id ORDER BY u.created_at").all(),
    ...
    env.DB.prepare("SELECT sca.user_id AS userId,sca.class_id AS classId FROM staff_class_access sca JOIN classes c ON c.id=sca.class_id WHERE c.status='active'").all(),
...
export async function POST(request: Request) {
  ...
  if (access.authType !== "teacher_admin") return Response.json({ error: "只有主教师可以修改工作室成员与权限" }, { status: 403 });
  const body = await request.json() as Record<string, unknown>, action = String(body.action || "upsertUser");
  if (action === "upsertUser") {
    const email = String(body.email || "").trim().toLowerCase(), name = String(body.name || "").trim(), role = String(body.role || "") as RoleCode, studentId = Number(body.studentId || 0), password = String(body.password || "");
    if (!email || !name || !allowedRoles.includes(role)) return Response.json({ error: "姓名、邮箱和角色为必填项" }, { status: 400 });
    const existing = await env.DB.prepare("SELECT u.id,EXISTS(SELECT 1 FROM staff_credentials sc WHERE sc.user_id=u.id) AS hasCredential FROM users u WHERE lower(u.email)=lower(?)").bind(email).first<{ id: number; hasCredential: number }>();
    if (role === "assistant" && !existing?.hasCredential && !password) return Response.json({ error: "新助教必须设置初始登录密码" }, { status: 400 });
    if (password) { const strengthError = passwordStrengthError(password); if (strengthError) return Response.json({ error: strengthError }, { status: 400 }); }
    ...
    if (!roleRow) return Response.json({ error: "角色配置不存在" }, { status: 500 });
    ...
    if (role === "assistant" && password) { const credential = await setStaffPassword(target.id, password); if (!credential.ok) return Response.json({ error: credential.error }, { status: 400 }); }
    if (role !== "assistant") await env.DB.prepare("DELETE FROM staff_credentials WHERE user_id=?").bind(target.id).run();
    await audit(access, "assign_role", "user", target.id, { role, linkedStudentId: studentId || null, credentialChanged: Boolean(password) });
    return Response.json({ ok: true, userId: target.id, canSignIn: role === "assistant" });
  }
  if (action === "disableUser") {
    const userId = Number(body.userId || 0); if (!userId || userId === access.id) return Response.json({ error: "不能停用当前登录账号" }, { status: 400 });
    const target = await env.DB.prepare("SELECT email,status FROM users WHERE id=?").bind(userId).first<{ email: string; status: string }>();
  ...
  if (action === "setClassAccess") {
    ...
    const target = await env.DB.prepare("SELECT u.status,r.code FROM users u JOIN user_roles ur ON ur.user_id=u.id JOIN roles r ON r.id=ur.role_id WHERE u.id=?").bind(userId).first<{ status: string; code: string }>();
    if (!userId || target?.status !== "active" || target?.code !== "assistant") return Response.json({ error: "只能为已启用的助教分配班级" }, { status: 400 });
