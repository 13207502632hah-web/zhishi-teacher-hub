...
const importStageError = (stage: string, error: unknown) =>
  new Error(`${stage}：${error instanceof Error ? error.message : "未知错误"}`);
...
export async function importQuestionSetForAccess(access: import("../../../../lib/access").AccessContext, body: QuestionImportBody) {
  ...
  for (const ref of uniqueSourceRefs(sourceRefs).filter((item) => existing.has(item.fingerprint))) {
    ...
    // A source fingerprint proves file identity, not parse completeness or absence of teacher edits.
    // Reimports may fill blanks but cannot replace content or retire questions missing from this pass.
  ...
  if (!unique.length) return Response.json({ error: enrichedIds.length ? `所有题目均已存在；补齐 ${enrichedIds.length} 道` : "所有题目都与现有题库重复，未创建导入任务", existing: previous || undefined, duplicates: prepared.length, enriched: enrichedIds.length, enrichedFields }, { status: 409 });
  ...
  try {
    ({ candidates: comparisonPool, coverage: similarityCoverage } = await collectSimilarityCandidates(env.DB, unique));
  } catch (error) {
    throw importStageError("相似题候选检索失败", error);
  }
  ...
  try {
    ...
    throw importStageError("题目明细写入失败", error);
  }
