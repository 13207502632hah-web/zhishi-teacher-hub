...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  for (const item of valid) {
    ...
    const objectiveScore = item.objectiveScore === "" || item.objectiveScore == null ? null : Number(item.objectiveScore), subjectiveScore = item.subjectiveScore === "" || item.subjectiveScore == null ? null : Number(item.subjectiveScore), scoreIssue = validateAssessmentResult({ score, objectiveScore, subjectiveScore }, Number(project.total_score));
    if (scoreIssue) return Response.json({ error: `${studentId}号学生：${scoreIssue}` }, { status: 400 });
    if (score == null) { await env.DB.prepare("UPDATE exam_project_students SET assessment_result_id=NULL,status='pending',updated_at=CURRENT_TIMESTAMP WHERE project_id=? AND student_id=?").bind(id, studentId).run(); continue; }
    const result = await env.DB.prepare("INSERT INTO assessment_results(assessment_id,student_id,score,objective_score,subjective_score,teacher_note) VALUES(?,?,?,?,?,?) ON CONFLICT(assessment_id,student_id) DO UPDATE SET score=excluded.score,objective_score=excluded.objective_score,subjective_score=excluded.subjective_score,teacher_note=excluded.teacher_note,updated_at=CURRENT_TIMESTAMP RETURNING id").bind(assessment?.id, studentId, score, objectiveScore, subjectiveScore, String(item.teacherNote || "")).first<{ id: number }>();
