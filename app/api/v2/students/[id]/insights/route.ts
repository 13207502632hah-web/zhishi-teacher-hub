...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const timeline = [
    ...
    ...assignments.results.map((item) => ({ date: String(item.submittedAt || item.dueAt || item.createdAt).slice(0, 10), type: item.status === "corrected" || item.status === "revision" || item.status === "revision_submitted" ? "订正" : "作业", title: item.title, detail: item.status, href: `/v2/modules/assignments?q=${encodeURIComponent(String(item.title || ""))}&submissionStatus=${item.status}` })),
    ...assessments.results.map((item) => ({ date: item.date, type: "测验", title: item.title, detail: item.score == null ? "待录入" : `${item.score}/${item.totalScore}`, href: "/v2/operations?tab=assessments" })),
    ...
    ...observations.results.map((item) => ({ date: String(item.createdAt).slice(0, 10), type: "已确认反馈", title: "课程反馈", detail: String(item.content || "").slice(0, 120), href: `/v2/modules/learning?view=feedback&studentId=${id}&status=confirmed` })),
    ...wrongQuestions.results.map((item) => ({ date: String(item.masteredAt || item.occurredAt).slice(0, 10), type: "错题", title: String(item.stem || "政治错题").slice(0, 100), detail: item.status === "mastered" ? "已掌握" : `待巩固${item.knowledgePoints ? ` · ${item.knowledgePoints}` : ""}`, href: `/v2/detail/students/${id}#wrong-questions` })),
