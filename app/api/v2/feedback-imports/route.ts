...
export async function POST(request: Request) {
  ...
  const body = await request.json() as Record<string, unknown>, sourceText = String(body.sourceText || "").trim().slice(0, 100000), ocrText = String(body.ocrText || "").trim().slice(0, 100000), sourceAssetId = Number(body.sourceAssetId || 0) || null;
  ...
  if (sourceAssetId) { const source = await env.DB.prepare("SELECT id FROM file_assets WHERE id=? AND status='active' AND mime_type IN ('image/jpeg','image/png','image/webp') AND created_by=?").bind(sourceAssetId, access.id).first(); if (!source) return Response.json({ error: "反馈原图不存在、格式不受支持或无权使用" }, { status: 403 }); }
