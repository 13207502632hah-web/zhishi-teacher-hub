...
export async function POST(request: Request) {
  ...
  const [resource] = await getDb().insert(resources).values({ title: String(body.title).trim(), type: String(body.type || "备课素材"), url, tags: String(body.tags || ""), content: String(body.content || ""), sourceRef: String(body.sourceRef || "manual"), visibility: "private", ownerId: access.id }).returning();
