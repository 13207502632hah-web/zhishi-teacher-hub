...
export async function GET(request: Request) {
  ...
  const [lessons, feedback, results, finance, records] = await env.DB.batch([
    ...
    env.DB.prepare("SELECT id,kind,title,content,occurred_at AS occurredAt FROM v2_mobile_records WHERE status='confirmed' AND deleted_at IS NULL AND (audience='both' OR audience=?) AND (student_id=? OR class_id IN (SELECT class_id FROM enrollments WHERE student_id=? AND status='active')) ORDER BY occurred_at DESC LIMIT 20").bind(access.role, studentId, studentId),
  ]);
  return Response.json({ studentId, lessons: lessons.results, feedback: feedback.results, results: results.results, finance: finance.results, records: records.results });
}
