...
export async function GET(request: Request) {
  ...
  const assignmentFilters = new URLSearchParams({ kind: "homework" });
  const [assignments, me] = await Promise.all([listAssignments({ kind: "mini", access }, assignmentFilters), miniAccountState(access, access.expiresAt)]);
