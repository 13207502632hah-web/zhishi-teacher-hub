...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  for (const row of rows.results) {
    ...
    submissions.push({ ...row, attachments: assets.results.map((asset) => ({ ...asset, url: `/api/v2/files/${asset.id}` })), annotations: annotations.results });
  }
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (body.action !== "save-review") return Response.json({ error: "正式批改请提交到待确认中心" }, { status: 400 });
