...
async function list(accessId: number) {
  ...
  const [rows, tasks] = await Promise.all([
    env.DB.prepare("SELECT r.id,r.task_id AS taskId,r.question_id AS questionId,r.source_updated_at AS sourceUpdatedAt,r.current_values_json AS currentValuesJson,r.safe_suggestions_json AS safeSuggestionsJson,r.sensitive_suggestions_json AS sensitiveSuggestionsJson,r.confidence_json AS confidenceJson,r.reasons_json AS reasonsJson,r.eligible_fields_json AS eligibleFieldsJson,r.status,r.applied_fields_json AS appliedFieldsJson,r.created_at AS createdAt,q.stem,q.status AS questionStatus FROM ai_question_reviews r JOIN ai_runs ar ON ar.id=r.run_id JOIN questions q ON q.id=r.question_id WHERE ar.user_id=? AND r.status IN ('pending','partially_applied') ORDER BY r.created_at DESC LIMIT 100").bind(accessId).all<Record<string, any>>(),
