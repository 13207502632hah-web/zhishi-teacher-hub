...
export async function POST(request: Request) {
  ...
  if (body.action === "queue") {
    let queued = 0;
    for (const id of ids) { const owned = await env.DB.prepare("SELECT 1 AS owned FROM ai_question_reviews r JOIN ai_runs ar ON ar.id=r.run_id WHERE r.id=? AND ar.user_id=? AND r.status IN ('pending','partially_applied')").bind(id, access.id).first(); if (!owned) continue; await env.DB.prepare("UPDATE ai_question_reviews SET status='approval_pending',updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(id).run(); await audit(access, "queue_approval", "ai_question_review", id); queued += 1; }
