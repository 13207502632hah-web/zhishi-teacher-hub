...
export async function GET(request: Request) {
  ...
  const [results, reviewSummary, aiUsage] = await Promise.all([db.batch([
    ...
    scoped(`SELECT l.id,l.date,l.start_time AS startTime,l.end_time AS endTime,l.course_name AS courseName,l.topic,l.mode,l.location,l.online_link AS onlineLink,l.status,c.name AS className,CASE WHEN TRIM(COALESCE(l.teaching_goals,''))<>'' AND TRIM(COALESCE(l.key_points,''))<>'' AND TRIM(COALESCE(l.materials,''))<>'' THEN 1 ELSE 0 END AS prepReady FROM lessons l LEFT JOIN classes c ON c.id=l.class_id WHERE l.date>? AND l.date<=? AND l.status!='cancelled'${lessonScope} ORDER BY l.date,l.start_time LIMIT 30`, [today, horizonDate]),
    scoped(`SELECT l.id,l.date,l.start_time AS startTime,l.end_time AS endTime,l.course_name AS courseName,l.topic,l.mode,l.location,l.online_link AS onlineLink,l.status,c.name AS className,l.updated_at AS updatedAt FROM lessons l LEFT JOIN classes c ON c.id=l.class_id WHERE l.date<? AND l.status IN ('draft','scheduled','makeup','rescheduled')${lessonScope} ORDER BY l.date,l.start_time LIMIT 12`, [today]),
    ...
    scoped(`SELECT l.id,l.date,l.start_time AS startTime,l.end_time AS endTime,l.course_name AS courseName,l.topic,l.mode,l.location,l.online_link AS onlineLink,l.status,c.name AS className,CASE WHEN TRIM(COALESCE(l.teaching_goals,''))<>'' AND TRIM(COALESCE(l.key_points,''))<>'' AND TRIM(COALESCE(l.materials,''))<>'' THEN 1 ELSE 0 END AS prepReady FROM lessons l LEFT JOIN classes c ON c.id=l.class_id WHERE (l.date>? OR (l.date=? AND COALESCE(NULLIF(l.end_time,''),NULLIF(l.start_time,''),'23:59')>=?)) AND l.status NOT IN ('cancelled','completed')${lessonScope} ORDER BY l.date,l.start_time LIMIT 1`, [today, today, currentTime]),
  ...
  const lessonIds = [...new Set([...rows(6), ...rows(15), ...rows(16), ...rows(19)].map((row) => Number(row.id)).filter(Boolean))], displayNames = new Map<number, string>(), courseTypes = new Map<number, string>();
  if (lessonIds.length) { const marks = lessonIds.map(() => "?").join(","), named = await db.prepare(`SELECT l.id,c.course_type AS courseType,GROUP_CONCAT(s.name,'、') AS studentNames FROM lessons l LEFT JOIN classes c ON c.id=l.class_id LEFT JOIN enrollments e ON e.class_id=l.class_id AND e.status='active' LEFT JOIN students s ON s.id=e.student_id AND s.status='active' WHERE l.id IN (${marks}) GROUP BY l.id,c.course_type`).bind(...lessonIds).all<Record<string, unknown>>(); named.results.forEach((row) => { displayNames.set(Number(row.id), String(row.studentNames || "")); courseTypes.set(Number(row.id), String(row.courseType || "")); }); }
  const display = (row: Record<string, any>): Record<string, any> => { const courseType = courseTypes.get(Number(row.id)) || "", fields = lessonDisplay({ ...row, studentNames: displayNames.get(Number(row.id)) || "", courseType }); return { ...row, originalClassName: row.className || "", className: fields.displaySubject || row.className || courseType, courseType, ...fields }; };
  ...
  for (const lesson of overdueLessons) actions.push({ key: `overdue-${lesson.id}`, type: "lesson-closeout", title: `补记：${lesson.displaySubject || lesson.topic || lesson.courseName}`, reason: "课时日期已过，课堂记录尚未完成", dueAt: lesson.date, href: `/v2/detail/lessons/${lesson.id}`, updatedAt: lesson.updatedAt });
  for (const lesson of todayLessons) if (lesson.status !== "completed") actions.push({ key: `today-${lesson.id}`, type: "today-lesson", title: `${lesson.prepReady ? "完成" : "备课"}：${lesson.displaySubject || lesson.topic || lesson.courseName}`, reason: lesson.prepReady ? "今天的课尚未完成课后记录" : "教学目标、重点或资料清单尚未补齐", dueAt: today, href: `/v2/detail/lessons/${lesson.id}` });
  if (nextLesson && !Number(nextLesson.prepReady) && nextLesson.date !== today) actions.push({ key: `next-${nextLesson.id}`, type: "lesson-prep", title: `准备下一节：${nextLesson.displaySubject || nextLesson.topic || nextLesson.courseName}`, reason: "下一节课的教学目标、重点或资料清单尚未补齐", dueAt: nextLesson.date, href: `/v2/detail/lessons/${nextLesson.id}` });
  for (const item of rows(20)) actions.push({ key: `assignment-${item.id}`, type: "assignment", title: `处理作业：${item.title}`, reason: `${item.pendingCount} 名学生待完成、订正或批改`, dueAt: String(item.dueAt || ""), href: `/v2/modules/assignments?lessonId=${item.lessonId || ""}&submissionStatus=pending`, updatedAt: item.updatedAt });
  for (const item of rows(21)) actions.push({ key: `feedback-${item.id}`, type: "feedback", title: `确认反馈：${item.subject}`, reason: "反馈仍为草稿，尚未确认", dueAt: String(item.dueAt || ""), href: `/v2/modules/learning?view=feedback&lessonId=${item.lessonId || ""}&status=draft`, updatedAt: item.updatedAt });
  for (const item of rows(22)) actions.push({ key: `finance-${item.id}`, type: "finance", title: `核对结算：${item.topic || item.courseName}`, reason: "课时结算仍处于待核对状态", dueAt: String(item.dueAt || ""), href: `/v2/modules/finance?lessonId=${item.lessonId}&status=review`, updatedAt: item.updatedAt });
  for (const item of rows(9)) actions.push({ key: `student-${item.id}`, type: "student", title: `跟进学生：${item.name}`, reason: String(item.riskTags || "已有教师确认的关注事项"), dueAt: "", href: `/v2/detail/students/${item.id}`, updatedAt: item.updatedAt });
  ...
  if (access.role === "teacher" && today.slice(5, 7) === "09") {
    ...
    if (promotionDue) actions.push({ key: `promotion-${academicYear}`, type: "grade-promotion", title: "核对新学年年级晋升", reason: "9月已生成晋升建议；排除留级、转学或暂缓学生后再确认", dueAt: today, href: `/v2/operations?tab=academic&year=${academicYear}` });
  }
