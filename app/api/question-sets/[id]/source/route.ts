...
import { hasQuestionSetClassAccess } from "../../../../lib/question-source-access";
...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!record) return Response.json({ error: "题组不存在" }, { status: 404 });
  if (access.role === "assistant" && !(await hasQuestionSetClassAccess(env.DB, id, access.id))) {
    return Response.json({ error: "当前账号未获授权访问该题组原始文件" }, { status: 403 });
  }
