...
export async function POST() {
  ...
  if (existing) return Response.json({ error: "当前工作区已有演示数据，请先清除后再创建新的演示数据" }, { status: 409 });
  ...
  await audit(access, "seed_demo", "demo", runId, { classes: 2, students: studentIds.length, lessons: lessonIds.length, questions: questionIds.length + 1, papers: 2, assessment: Boolean(assessment), reviewSet: Boolean(reviewSet) });
  return Response.json({ ok: true, runId, summary: { classes: 2, students: studentIds.length, lessons: lessonIds.length, questions: questionIds.length + 1, papers: 2, feedback: Math.min(8, completedLessons.length), reflections: Math.min(6, completedLessons.length), assessments: assessment ? 1 : 0, wrongQuestions: wrongCount, reviewSets: reviewSet ? 1 : 0 } }, { status: 201 });
}
...
export async function DELETE(request: Request) {
  ...
  const ids = (type: string) => records.results.filter((row) => row.entityType === type).map((row) => row.entityId), questionIds = ids("question"), lessonIds = ids("lesson"), paperIds = ids("paper"), assessmentIds = ids("assessment"), wrongQuestionIds = ids("wrong_question"), questionSetIds = ids("question_set"), studentIds = ids("student"), classIds = ids("class"), marks = (values: number[]) => values.map(() => "?").join(",") || "NULL";
  const statements = [
    ...
    env.DB.prepare(`DELETE FROM paper_questions WHERE paper_id IN (${marks(paperIds)})`).bind(...paperIds), env.DB.prepare(`DELETE FROM papers WHERE id IN (${marks(paperIds)})`).bind(...paperIds), env.DB.prepare(`DELETE FROM lesson_questions WHERE lesson_id IN (${marks(lessonIds)})`).bind(...lessonIds), env.DB.prepare(`DELETE FROM wrong_questions WHERE id IN (${marks(wrongQuestionIds)}) OR student_id IN (${marks(studentIds)})`).bind(...wrongQuestionIds, ...studentIds), env.DB.prepare(`DELETE FROM assessment_results WHERE assessment_id IN (${marks(assessmentIds)})`).bind(...assessmentIds), env.DB.prepare(`DELETE FROM assessments WHERE id IN (${marks(assessmentIds)})`).bind(...assessmentIds), env.DB.prepare(`DELETE FROM attendance WHERE lesson_id IN (${marks(lessonIds)})`).bind(...lessonIds), env.DB.prepare(`DELETE FROM student_lesson_records WHERE lesson_id IN (${marks(lessonIds)})`).bind(...lessonIds), env.DB.prepare(`DELETE FROM assignment_submissions WHERE student_id IN (${marks(studentIds)})`).bind(...studentIds), env.DB.prepare(`DELETE FROM assignments WHERE lesson_id IN (${marks(lessonIds)})`).bind(...lessonIds), env.DB.prepare(`DELETE FROM feedback WHERE id IN (${marks(ids("feedback"))})`).bind(...ids("feedback")), env.DB.prepare(`DELETE FROM reflections WHERE id IN (${marks(ids("reflection"))})`).bind(...ids("reflection")), env.DB.prepare(`DELETE FROM questions WHERE id IN (${marks(questionIds)})`).bind(...questionIds), env.DB.prepare(`DELETE FROM question_sets WHERE id IN (${marks(questionSetIds)})`).bind(...questionSetIds), env.DB.prepare(`DELETE FROM lessons WHERE id IN (${marks(lessonIds)})`).bind(...lessonIds), env.DB.prepare(`DELETE FROM enrollments WHERE student_id IN (${marks(studentIds)}) OR class_id IN (${marks(classIds)})`).bind(...studentIds, ...classIds), env.DB.prepare(`DELETE FROM staff_class_access WHERE class_id IN (${marks(classIds)})`).bind(...classIds), env.DB.prepare(`DELETE FROM students WHERE id IN (${marks(studentIds)})`).bind(...studentIds), env.DB.prepare(`DELETE FROM classes WHERE id IN (${marks(classIds)})`).bind(...classIds), env.DB.prepare("DELETE FROM demo_records"),
  ];
