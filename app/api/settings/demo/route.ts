...
export async function DELETE(request: Request) {
  ...
  const statements = [
    ...
    env.DB.prepare(`UPDATE lesson_workflow_state SET homework_assignment_id=NULL WHERE homework_assignment_id IN (${marks(assignmentIds)})`).bind(...assignmentIds),
    env.DB.prepare(`UPDATE lesson_workflow_state SET homework_paper_id=NULL WHERE homework_paper_id IN (${marks(paperIds)})`).bind(...paperIds),
    ...
    env.DB.prepare(`UPDATE assignments SET paper_id=NULL WHERE paper_id IN (${marks(paperIds)})`).bind(...paperIds),
    env.DB.prepare(`DELETE FROM export_jobs WHERE paper_id IN (${marks(paperIds)})`).bind(...paperIds),
    env.DB.prepare(`DELETE FROM paper_files WHERE paper_id IN (${marks(paperIds)})`).bind(...paperIds),
    ...
    env.DB.prepare(`UPDATE assessments SET paper_id=NULL WHERE paper_id IN (${marks(paperIds)})`).bind(...paperIds),
    ...
    env.DB.prepare(`UPDATE question_sets SET paper_id=NULL WHERE paper_id IN (${marks(paperIds)})`).bind(...paperIds),
    env.DB.prepare(`UPDATE exam_projects SET paper_id=NULL WHERE paper_id IN (${marks(paperIds)})`).bind(...paperIds),
