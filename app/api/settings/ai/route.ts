...
async function storedSettings(userId: number) {
  await env.DB.prepare("INSERT OR IGNORE INTO ai_settings(user_id) VALUES(?)").bind(userId).run();
  return env.DB.prepare("SELECT enabled,include_student_name AS includeStudentName,privacy_ack_at AS privacyAckAt,daily_limit AS dailyLimit,emergency_disabled AS emergencyDisabled FROM ai_settings WHERE user_id=?").bind(userId).first<StoredAiSettings>();
}
