...
type StoredAiSettings = { enabled: number | string; includeStudentName: number | string; privacyAckAt: string | null; dailyLimit: number | string; emergencyDisabled: number | string };

async function storedSettings(userId: number) {
  const settings = await env.DB.prepare("SELECT enabled,include_student_name AS includeStudentName,privacy_ack_at AS privacyAckAt,daily_limit AS dailyLimit,emergency_disabled AS emergencyDisabled FROM ai_settings WHERE user_id=?").bind(userId).first<StoredAiSettings>();
  return settings ?? { enabled: 0, includeStudentName: 1, privacyAckAt: null, dailyLimit: 50, emergencyDisabled: 0 };
}
...
export async function PATCH(request: Request) {
  ...
  const requestedLimit = body.dailyLimit === undefined ? Number(existing.dailyLimit) : Number(body.dailyLimit);
  const safeLimit = Number.isFinite(requestedLimit) ? requestedLimit : Number(existing.dailyLimit);
  const limit = Math.min(200, Math.max(1, safeLimit));
