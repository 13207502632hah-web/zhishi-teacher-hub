...
export async function confirmRecognition(access: AccessContext, jobId: number) {
  ...
  if (job.stage === "confirmed") return Response.json({ ok: true, alreadyConfirmed: true, score: scoreOf(allItems), count: allItems.length });
...
export async function POST(request: Request) {
  ...
  return confirmRecognition(access, jobId);
}
