...
export async function GET(request: Request) {
  ...
  const sql = `SELECT c.*, (SELECT COUNT(*) FROM enrollments e WHERE e.class_id=c.id AND e.status='active') AS studentCount, (SELECT COUNT(*) FROM lessons l WHERE l.class_id=c.id) AS lessonCount, (SELECT COUNT(DISTINCT slr.student_id) FROM student_lesson_records slr JOIN lessons l ON l.id=slr.lesson_id WHERE l.class_id=c.id AND slr.risk_confirmed=1) AS riskCount FROM classes c ${where.length ? `WHERE ${where.join(" AND ")}` : ""} ORDER BY CASE c.status WHEN 'active' THEN 0 ELSE 1 END,c.updated_at DESC`;
