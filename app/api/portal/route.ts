...
async function linkedStudents(access: { portalAccountId: number; role: PortalRole }) {
  const rows = access.role === "student"
    ? await env.DB.prepare("SELECT DISTINCT s.id,s.name,s.grade,s.stage_goal AS stageGoal FROM students s WHERE s.status='active' AND (EXISTS(SELECT 1 FROM mini_bindings mb WHERE mb.account_id=? AND mb.student_id=s.id AND mb.role='student' AND mb.status='active') OR EXISTS(SELECT 1 FROM wechat_accounts wa WHERE wa.id=? AND wa.role='student' AND wa.status='active' AND wa.student_id=s.id)) ORDER BY s.name")
      .bind(access.portalAccountId, access.portalAccountId).all<Record<string, unknown>>()
    : await env.DB.prepare("SELECT DISTINCT s.id,s.name,s.grade,s.stage_goal AS stageGoal FROM students s WHERE s.status='active' AND (EXISTS(SELECT 1 FROM mini_bindings mb WHERE mb.account_id=? AND mb.student_id=s.id AND mb.role='parent' AND mb.status='active') OR EXISTS(SELECT 1 FROM parent_student_links p WHERE p.parent_account_id=? AND p.student_id=s.id AND p.status='active')) ORDER BY s.name")
      .bind(access.portalAccountId, access.portalAccountId).all<Record<string, unknown>>();
...
async function bindingStatus(access: { portalAccountId: number; role: PortalRole }, hasStudents: boolean): Promise<BindingStatus> {
  if (hasStudents) return "active";
  const rows = await env.DB.prepare("SELECT wa.status AS accountStatus,mb.status AS bindingStatus FROM wechat_accounts wa LEFT JOIN mini_bindings mb ON mb.account_id=wa.id AND mb.role=? WHERE wa.id=? ORDER BY mb.status='active' DESC,mb.updated_at DESC").bind(access.role, access.portalAccountId).all<{ accountStatus: string; bindingStatus: string | null }>();
  const legacyParent = access.role === "parent"
    ? await env.DB.prepare("SELECT status FROM parent_student_links WHERE parent_account_id=? ORDER BY updated_at DESC").bind(access.portalAccountId).all<{ status: string }>()
    : { results: [] as { status: string }[] };
  if (rows.results.some((row) => row.accountStatus === "disabled" || row.bindingStatus === "disabled") || legacyParent.results.some((row) => row.status === "disabled")) return "disabled";
...
async function protectedFile(request: Request, studentIds: number[]) {
  ...
  if (!marks) return json({ error: "文件不存在或无权查看" }, { status: 404 });
  ...
  if (!meta || !allowed) return json({ error: "文件不存在或无权查看" }, { status: 404 });
...
export async function GET(request: Request) {
  ...
  if (!access.portalAccountId) return json({ error: "门户账号信息不完整，请重新从微信小程序进入", code: "PORTAL_ACCOUNT_REQUIRED" }, { status: 403 });
  const portalAccess = { portalAccountId: access.portalAccountId, role };
