...
export async function POST(request: Request) {
  ...
  const db = getDb(), [paper] = await db.insert(papers).values({ title: title || file.name.replace(/\.[^.]+$/, ""), type: String(form.get("type") || "完整试卷"), stage: String(form.get("stage") || ""), grade: String(form.get("grade") || ""), textbookVersion: String(form.get("textbookVersion") || ""), year: Number(form.get("year") || 0) || null, region: String(form.get("region") || ""), school: String(form.get("school") || ""), source: String(form.get("source") || ""), tags: String(form.get("tags") || ""), useStatus: "unused", parseStatus: "queued", status: "completed" }).returning();
