...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return Response.json({ content: row, missing: { answer: !String(row.answer || "").trim(), analysis: !String(row.analysis || row.answerPoints || row.scoringPoints || row.standardExpression || "").trim() } }, { headers: { "Cache-Control": "private, no-store" } });
}
