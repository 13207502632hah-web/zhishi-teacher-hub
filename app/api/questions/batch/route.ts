...
export async function POST(request: Request) {
  ...
  if (action === "tags") updates.tags = String(body.value || "").trim();
    ...
    const blocked = selected.filter((item) => !item.stem || !item.answer || !item.analysis || !item.knowledgePoints || Number(item.parseConfidence ?? 1) < .7).map((item) => ({ id: item.id, issues: [!item.answer && "缺少答案", !item.analysis && "缺少解析", !item.knowledgePoints && "缺少知识点", Number(item.parseConfidence ?? 1) < .7 && "识别置信度低"].filter(Boolean) }));
