...
type QueueItem = { key: string; name: string; status: "waiting" | "processing" | "completed" | "duplicate" | "failed"; message: string; setId?: number; count?: number; file?: File };
...
type QuestionHealthSummary = { total: number; missingAnswer: number; missingAnalysis: number; useCount: number };
...
export default function QuestionsPage() {
  ...
  const loadAiReviews = useCallback(async () => {
    aiReviewRequest.current?.abort();
    if (!ready) { setAiReviewState("idle"); return; }
    const controller = new AbortController();
    aiReviewRequest.current = controller;
    ...
    try {
      ...
      if (controller.signal.aborted) return;
      setAiReviewError(reason instanceof Error ? reason.message : "AI 复核列表读取失败");
      setAiReviewState("error");
    }
  }, [ready]);
  useEffect(() => { if (ready) void loadAiReviews(); return () => aiReviewRequest.current?.abort(); }, [ready, loadAiReviews]);
  const loadFacets = useCallback(async () => {
    facetRequest.current?.abort();
    if (!ready) { setFacetState("idle"); return; }
    const controller = new AbortController();
    facetRequest.current = controller;
    const params = new URLSearchParams({ status, stage, grade, textbookVersion, volume, unit });
    setFacetState("loading");
    setFacetError("");
    try {
      const data = await requestJson<QuestionFacetsResponse>(`/api/questions/facets?${params}`, { signal: controller.signal, cache: "no-store" });
      if (!data?.facets || typeof data.facets !== "object" || Array.isArray(data.facets) || !Object.values(data.facets).every(Array.isArray)) throw new HttpError(200, "教材目录筛选接口没有返回完整数据");
      setFacets(data.facets);
      setFacetState("ready");
    } catch (reason) {
      if (controller.signal.aborted) return;
      setFacetError(reason instanceof Error ? reason.message : "教材目录筛选读取失败");
      setFacetState("error");
    }
  }, [ready, status, stage, grade, textbookVersion, volume, unit]);
  useEffect(() => { void loadFacets(); return () => facetRequest.current?.abort(); }, [loadFacets]);
  const loadHealth = useCallback(async () => {
    healthRequest.current?.abort();
    if (!ready || status !== "active") { setHealthState("idle"); return; }
    const controller = new AbortController();
    healthRequest.current = controller;
    const params = new URLSearchParams({ stage, grade, textbookVersion, volume, unit, topic, knowledge: appliedSearch.knowledge });
    setHealthState("loading");
    setHealthError("");
    try {
      const data = await requestJson<QuestionHealthResponse>(`/api/questions/stats?${params}`, { signal: controller.signal, cache: "no-store" });
      if (!data?.summary || !Array.isArray(data.knowledge)) throw new HttpError(200, "题库健康指标接口没有返回完整数据");
      setHealth(data);
      setHealthState("ready");
    } catch (reason) {
      if (controller.signal.aborted) return;
      setHealthError(reason instanceof Error ? reason.message : "题库健康指标读取失败");
      setHealthState("error");
    }
  }, [ready, status, stage, grade, textbookVersion, volume, unit, topic, appliedSearch.knowledge]);
  ...
  const runQueue = async (items: QueueItem[]) => {
    ...
    for (const item of items) {
      ...
      if (!file) { update(item.key, { status: "failed", message: "刷新后浏览器不保留本地文件，请重新选择同名文件重试" }); continue; }
      update(item.key, { status: "processing", message: "正在保存并解析" });
      try {
  ...
  return <AppShell title="题库与组卷" subtitle="政治学科题目、人工校对与试卷草稿" actions={<><a className="secondaryButton" href={`/api/questions/portable?format=json&status=${status}`}>导出题库 JSON</a><a className="secondaryButton" href={`/api/questions/portable?format=csv&status=${status}`}>导出 CSV</a><label className="secondaryButton">导入 JSON<input hidden disabled={Boolean(importAction)} type="file" accept="application/json,.json" onChange={(event) => void importPortable(event.target.files?.[0])} /></label><label className="primaryButton">批量导入 Word<input hidden disabled={queueRunning || Boolean(importAction)} multiple type="file" accept=".docx" onChange={(event) => selectQueueFiles(event.target.files)} /></label><Link className="secondaryButton" href="/papers">组卷工作台</Link><button className="secondaryButton" disabled={Boolean(questionAction)} onClick={() => edit()}>＋ 手动录题</button></>}>
    ...
    {queue.some((item) => item.status === "waiting" && item.file) && <div className="queueStart"><b>已选择 {queue.filter((item) => item.status === "waiting" && item.file).length} 个待处理文件</b><button className="primaryButton" disabled={queueRunning} onClick={() => void runQueue(queue.filter((item) => item.status === "waiting" && item.file))}>{queueRunning ? "正在逐文件处理…" : "确认目录并开始队列"}</button></div>}
    {duplicateReport?.similar?.length ? <section className="panel duplicateCompare"><div className="panelTitle"><div><p>只提示，不自动删除</p><h2>相似题并排核对</h2></div><b>{duplicateReport.similar.length} 组</b></div>{duplicateReport.similar.map((item, index) => <article key={`${item.candidateId}-${index}`}><div><span>本次导入</span><p>{String(item.sourceStem || "")}</p></div><b>{Math.round(Number(item.similarity || 0) * 100)}% 相似</b><div><span>题库 #{String(item.candidateId || "")}</span><p>{String(item.candidateStem || "")}</p></div></article>)}</section> : null}
    {queue.length > 0 && <section className="panel importQueue" aria-label="Word 批量导入队列"><div className="panelTitle"><div><p>可恢复任务</p><h2>Word 导入队列</h2></div><b>{queue.filter((item) => item.status === "completed").length}/{queue.length} 完成</b></div><div className="queueMeta"><label>学段<select value={importMeta.stage} onChange={(event) => setImportMeta({ ...importMeta, stage: event.target.value, grade: event.target.value === "高中" ? "高一" : "七年级" })}><option>初中</option><option>高中</option></select></label><label>年级<select value={importMeta.grade} onChange={(event) => setImportMeta({ ...importMeta, grade: event.target.value })}>{(importMeta.stage === "高中" ? ["高一", "高二", "高三"] : ["七年级", "八年级", "九年级"]).map((item) => <option key={item}>{item}</option>)}</select></label><label>教材版本<input value={importMeta.textbookVersion} onChange={(event) => setImportMeta({ ...importMeta, textbookVersion: event.target.value })} /></label><label>册别/模块<input value={importMeta.volume} onChange={(event) => setImportMeta({ ...importMeta, volume: event.target.value })} /></label><label>单元<input value={importMeta.unit} onChange={(event) => setImportMeta({ ...importMeta, unit: event.target.value })} /></label><label>课/框<input value={importMeta.topic} onChange={(event) => setImportMeta({ ...importMeta, topic: event.target.value })} /></label></div><p className="helperText">队列按文件顺序执行；单个文件失败不会中断后续文件。成功任务刷新后仍可继续校对，浏览器不会永久保存尚未上传的本地文件。</p><div className="queueList">{queue.map((item) => <article key={item.key}><span className={`queueStatus ${item.status}`}>{item.status === "waiting" ? "等待" : item.status === "processing" ? "处理中" : item.status === "completed" ? "已建任务" : item.status === "duplicate" ? "已跳过重复" : "失败"}</span><div><b>{item.name}</b><small>{item.message}</small></div>{item.setId && <Link href={`/questions?import=1&set=${item.setId}`}>继续校对</Link>}{item.status === "failed" && item.file && <button disabled={queueRunning} onClick={() => void runQueue([item])}>重试</button>}</article>)}</div></section>}
    ...
    {tab === "library" && <>
      <div className="toolbar questionFilters secondaryFilters" aria-busy={facetState === "loading"}>
        <select aria-label="教材版本" value={textbookVersion} onChange={(event) => { setTextbookVersion(event.target.value); setVolume(""); setUnit(""); setTopic(""); setPage(1); }}><option value="">全部教材版本</option>{(facets.textbook_version || []).map((item) => <option key={String(item)}>{item}</option>)}</select>
        <select aria-label="册别或模块" value={volume} onChange={(event) => { setVolume(event.target.value); setUnit(""); setTopic(""); setPage(1); }}><option value="">全部册别/模块</option>{(facets.volume || []).map((item) => <option key={String(item)}>{item}</option>)}</select>
        <select aria-label="单元" value={unit} onChange={(event) => { setUnit(event.target.value); setTopic(""); setPage(1); }}><option value="">全部单元</option>{(facets.unit || []).map((item) => <option key={String(item)}>{item}</option>)}</select>
        <select aria-label="课或框" value={topic} onChange={(event) => { setTopic(event.target.value); setPage(1); }}><option value="">全部课/框</option>{(facets.topic || []).map((item) => <option key={String(item)}>{item}</option>)}</select>
        <button onClick={clearFilters}>清空全部筛选</button>
      </div>
      {facetState === "loading" && <div className="facetState" role="status">正在读取教材目录筛选…</div>}
      {facetState === "error" && <div className="facetState error" role="alert"><b>教材目录筛选读取失败</b><span>{facetError}</span>{hasFacetData && <em>教材目录选项来自上次成功读取，可继续使用或重试更新。</em>}<button onClick={() => void loadFacets()} type="button">重新读取教材目录</button></div>}
      <section className="savedSearchBar" aria-busy={savedViewState === "loading"} aria-label="保存和最近筛选"><div><input aria-label="筛选方案名称" value={savedName} onChange={(event) => setSavedName(event.target.value)} placeholder="给当前筛选命名" /><button disabled={Boolean(savedViewBusy)} onClick={saveView} type="button">{savedViewBusy === "save" ? "正在保存…" : "保存筛选"}</button></div>{savedViewState === "loading" && <div className="savedViewStatus" role="status">正在读取筛选方案…</div>}{savedViewState === "error" && <div className="savedViewStatus error" role="alert"><b>筛选方案读取失败</b><em>{savedViewError}</em><button disabled={Boolean(savedViewBusy)} onClick={() => void loadViews()} type="button">重新读取筛选方案</button></div>}{savedViews.map((view) => <span key={view.id}><button onClick={() => applyFilters(view.filters)} type="button">{view.name}</button><button aria-label={`删除筛选 ${view.name}`} disabled={Boolean(savedViewBusy)} onClick={() => void deleteView(view.id)} type="button">{savedViewBusy === `delete:${view.id}` ? "…" : "×"}</button></span>)}{recentSearches.slice(0, 3).map((item, index) => <button key={`recent-${index}`} onClick={() => applyFilters(item)} type="button">最近：{item.q || item.knowledge || item.topic || item.unit || item.stage || "组合筛选"}</button>)}</section>
    </>}
    {tab === "library" && status === "active" && healthState === "loading" && <div className="questionHealthState" role="status">正在读取题库健康指标…</div>}
    {tab === "library" && status === "active" && healthState === "error" && <div className="questionHealthState error" role="alert"><b>题库健康指标读取失败</b><span>{healthError}</span><button onClick={() => void loadHealth()} type="button">重新读取健康指标</button></div>}
    {tab === "library" && status === "active" && health && healthState !== "loading" && renderHealth(healthState === "error")}
    ...
    {tab === "library" && <>
      <div className="toolbar questionFilters">
        ...
        <select value={grade} onChange={(event) => { setGrade(event.target.value); setTextbookVersion(""); setVolume(""); setUnit(""); setTopic(""); setPage(1); }}><option value="">全部年级</option>{(facets.grade || grades).map((item) => <option key={String(item)}>{item}</option>)}</select>
        ...
        <input value={knowledge} onChange={(event) => setKnowledge(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") submitSearch(); }} placeholder="知识点" />
        ...
        <select value={region} onChange={(event) => { setRegion(event.target.value); setPage(1); }}><option value="">全部地区</option>{(facets.region || []).map((item) => <option key={String(item)}>{item}</option>)}</select>
