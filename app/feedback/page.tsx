...
const blank = () => ({ type: "lesson", audience: "private", lengthMode: "short", lessonId: "", studentId: "", classId: "", tone: "温和鼓励", opening: "", closing: "", styleRules: "", previousHomework: "", classPerformance: "", weakPoints: "", dueAt: "", customInput: "", learningContent: "", highlights: "", consolidate: "", homeworkRequirements: "", parentAdvice: "", nextFocus: "", periodStart: daysAgo(90), periodEnd: today(), periodSummary: "", progress: "", problems: "", goals: "", suggestions: "", content: "", shortContent: "", standardContent: "", aiGenerated: false, aiUncertainty: [] as string[], status: "draft", evidenceRefs: [] as Array<Record<string, any>> });
...
export default function FeedbackPage() {
  ...
  const save = async (status: string) => {
    ...
    const payload = { ...form, shortContent, standardContent, content: form.aiGenerated && form.content ? form.content : form.lengthMode === "standard" ? standardContent : shortContent, status };
  ...
  const generateAiDraft = async () => { if (!form.lessonId) { setMessage("请先选择关联课时"); return; } setBusy(true); setMessage("正在调用 DeepSeek 整理真实课堂记录…"); const response = await fetch("/api/ai/feedback-drafts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lessonId: form.lessonId, studentId: form.studentId, audience: form.audience, tone: form.tone, customInput: form.customInput }) }), result = await response.json(); setBusy(false); if (!response.ok) { setMessage(result.error || "AI 草稿生成失败"); return; } const draft = result.draft || {}, lesson = lessons.find((item) => item.id === Number(form.lessonId)); setForm({ ...form, ...draft, aiGenerated: true, aiUncertainty: draft.uncertainty || [], evidenceRefs: form.evidenceRefs?.length ? form.evidenceRefs : lesson ? [{ sourceType: "lesson", sourceId: lesson.id, label: `${lesson.date} ${lesson.topic || lesson.courseName}`, excerpt: lesson.actualContent || "", sourceDate: lesson.date }] : [] }); setMessage("DeepSeek 草稿已生成但尚未保存，请逐项核对后再保存或确认"); };
  ...
  return <AppShell title="课程反馈" subtitle="单节课与阶段反馈；生成后由教师逐项编辑确认" actions={<><Link className="secondaryButton" href="/feedback-imports">从反馈建立课时</Link><button className="primaryButton" onClick={() => { setEditing(null); setForm(blank()); setOpen(true); }}>＋ 新建反馈</button></>}>
    ...
    {open && <div className="modalBackdrop" role="presentation"><div className="lessonModal feedbackModal" role="dialog" aria-modal="true" aria-labelledby="feedback-title">
      ...
      <div className="formGrid">
        <div className="wide summaryAction"><button type="button" className="secondaryButton" onClick={applyTemplate}>使用{form.type === "lesson" ? "单节课" : "阶段"}反馈模板</button>{form.type === "lesson" && <button type="button" className="aiButton" disabled={busy || !form.lessonId} onClick={generateAiDraft}>{busy ? "DeepSeek 正在生成…" : "DeepSeek 生成课后闭环草稿"}</button>}<span>模板或 AI 只填入可编辑初稿，不替代真实课堂记录。</span></div>
