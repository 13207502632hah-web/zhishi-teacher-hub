...
const supportedTypes = new Set(["schedule-import", "question-import"]);
...
export async function runV2BackgroundJob(jobId: string) {
  ...
  if (!supportedTypes.has(claim.type)) {
    await requeueBackgroundJob(jobId, `后台消费者尚未注册任务类型：${claim.type}`);
    return { claimed: true, supported: false };
  }
  const access = await getBackgroundAccess(claim.userId);
  if (!access) {
    await requeueBackgroundJob(jobId, "任务创建者已停用或角色已失效");
    return { claimed: true, access: false };
  }
  try {
    if (claim.type === "schedule-import") await processScheduleImportJobV2(access, claim.id);
    else if (claim.type === "question-import") await processQuestionImportJobV2(access, claim.id);
    return { claimed: true, completed: true };
  } catch (reason) {
    const message = reason instanceof Error ? reason.message : "后台任务执行失败";
    return { claimed: true, completed: false, ...(await requeueBackgroundJob(jobId, message)) };
  }
}
