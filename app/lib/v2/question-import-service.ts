...
export async function createQuestionImportV2(access: AccessContext, form: FormData, operationId: string) {
  ...
  const buffer = await file.arrayBuffer(), answerBuffer = answerFile ? await answerFile.arrayBuffer() : null, fileFingerprint = await fingerprint(buffer), answerFingerprint = answerBuffer ? await fingerprint(answerBuffer) : "", sourceFingerprint = answerFingerprint ? await fingerprint(new TextEncoder().encode(`${fileFingerprint}:${answerFingerprint}`).buffer) : fileFingerprint, importId = crypto.randomUUID(), storageKey = `v2/question-imports/${new Date().toISOString().slice(0, 10)}/${importId}.${extension}`, answerStorageKey = answerFile ? `v2/question-imports/${new Date().toISOString().slice(0, 10)}/${importId}-answer.${answerExtension}` : "", created = await createJob(access, { type: "question-import", operationId, entityType: "question_set", entityId: importId, state: "queued", stage: "uploaded", payload: { fileName: file.name, mimeType: file.type || "application/octet-stream", extension, size: file.size, sourceFingerprint, storageKey, answerFileName: answerFile?.name || "", answerMimeType: answerFile?.type || "", answerExtension, answerSize: answerFile?.size || 0, answerStorageKey, name: String(form.get("name") || file.name.replace(/\.[^.]+$/, "")) } }); if (created.repeated) return { repeated: true, job: created.job };
  await Promise.all([
