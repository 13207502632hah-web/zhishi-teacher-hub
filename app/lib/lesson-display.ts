...
export function lessonDisplay(row: Record<string, unknown>) {
  ...
  const subject = names.length ? (names.length <= 3 ? names.join("、") : `${names.slice(0, 2).join("、")}等${names.length}人`) : text(row.courseType ?? row.course_type) || text(row.className ?? row.class_name) || text(row.courseName ?? row.course_name) || "学生待关联";
  ...
  return { studentNames: names, displaySubject: subject, displayLocation: location, displayTime: `${start}-${end}`, displayTitle: `${subject}——${location}——${start}-${end}` };
}
