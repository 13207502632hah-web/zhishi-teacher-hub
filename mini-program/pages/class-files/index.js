...
Page({
  ...
  async load() { this.setData({ loading: true, error: "" }); try { const me = await api.request("/api/v2/mini/me"), studentId = this.data.studentId || me.currentStudentId; if (!studentId) { this.setData({ me, loading: false, error: me.bindingStatus === "pending" ? "绑定申请待教师确认" : "请先绑定学生" }); return; } const data = await api.request(`/api/v2/mini/class-files?studentId=${studentId}`), studentIndex = Math.max(0, me.students.findIndex((item) => Number(item.studentId) === Number(studentId))); this.setData({ me, studentId, studentIndex, files: data.files || [], loading: false }); } catch (error) { this.setData({ loading: false, error: error.error || "班级资料加载失败" }); } },
